...
function initEvent(employee) {
    ...
    const popupConfirmQuit = new PopupJs("Pop-up thông báo", "Bạn có chắc muốn đóng form này không?", "<a>Tiếp tục</a>", "<button>Đóng</button>");
    ...
    $.ajax({
        ...
        $(".m-department").each(function() {
            ...
            dropdownList.empty();
            $.each(res, (i, item) => {
                const dropdownItem = `<div class="dropdown-item" value="${item.DepartmentId}"><i class="fas fa-check"></i>${item.DepartmentName}</div>`;
    ...
    $.ajax({
        ...
        $(".m-position").each(function() {
            ...
            dropdownList.empty();
            $.each(res, (i, item) => {
                const dropdownItem = `<div class="dropdown-item" value="${item.PositionId}"><i class="fas fa-check"></i>${item.PositionName}</div>`;
                dropdownList.append(dropdownItem);
            })
        })
    }).fail(error => {
 ...
 *  created by: Vũ Đức Tuấn 29/07/2021
 */
